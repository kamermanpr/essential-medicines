...
class.1<-with(drug.line.4, xtabs(~First.line)); class.1
class.1b<-(class.1/109)*100; round(class.1b,0); sum(class.1b)
class.2<-with(drug.line.4, xtabs(~Second.line)); class.2
class.2b<-(class.2/109)*100; round(class.2b,0); sum(class.2b)
#
# First line vs income (tab.2)
tab.2<-with(drug.line.4, xtabs(~First.line+World.Bank)); tab.2
...
tab.2c<-CMHtest(tab.2, type="cor"); tab.2c

# Second line vs income (tab.2nd.2)
tab.2nd.2<-with(drug.line.4, xtabs(~Second.line+World.Bank)); tab.2nd.2
...
tab.2nd.2c<-CMHtest(tab.2nd.2, type="cor"); tab.2nd.2c

#################
# Adjust p-values
#################
nam<-c("First line", "Second line"); nam
pval<-c(tab.2c$table[3], tab.2nd.2c$table[3]); pval
adj.pval<-p.adjust(pval, method="holm"); adj.pval
...
ami.4<-subset(ami.3, Ami.yes=="1" & TCA.yes=="1" & SNRI.yes=="0"& a2delta.yes=="0",
              select=c(World.Bank, Ami.yes, TCA.yes, SNRI.yes, a2delta.yes)); ami.4 # select countries with amitriptyline and no other first-line drugs
ami.5<-ami.4[!ami.4$World.Bank=="Undefined",]; ami.5 # remove "undefined" World Bank class
...
ami.7<-subset(ami.3, Ami.yes=="1" & TCA.yes=="1" & SNRI.yes=="0" & a2delta.yes=="0" & Tramadol.yes=="0" & Lido.yes=="0",
              select=c(World.Bank, Ami.yes, TCA.yes, SNRI.yes, a2delta.yes, Tramadol.yes, Lido.yes)); ami.7 # select countries with amitriptyline and no other first-line AND second-line drugs
ami.8<-ami.7[!ami.7$World.Bank=="Undefined",]; ami.8 # remove "undefined" World Bank class
...
# Figure
#########
first<-round(cbind(class.1b, tab.2.prop),1); first
colnames(first)[1]<-"All"; first

pdf("./figures/figure.pdf", paper='a4r')
par(mfrow=c(2,1))
par(mar=c(3,6,4,6))
par(xpd=T)
