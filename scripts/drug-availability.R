...
drug.avail.1<-ddply(data.dev, .(World.Bank), summarise,
            ...
            Valproate = sum(Listed=="Yes" & Drug=="Valproate")); drug.avail.1
drug.avail.2<-drug.avail.1

# Number of countries in each income category
Income<-with(data.dev, xtabs(~World.Bank+Drug)); Income[,-c(2:21)]
...
drug.avail.2
write.csv(drug.avail.2, "./data/drug-availability.csv", row.names = FALSE)
...
write.csv(drug.avail.2, "./data/drug-availability-percent.csv", row.names = FALSE)
